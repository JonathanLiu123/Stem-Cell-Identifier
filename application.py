...
# interactive portion, requires get and post methods
@app.route("/demonstration", methods=["GET", "POST"])
def demonstration():
    if request.method == "POST":
        # loading the model for testing/prediction
        model = keras.models.load_model("model.h5")

        # calling all of the form inputs and putting them into variables
        ...
        # ensuring that everything inputted is indeed a float, otherwise taking the user back with an error message
        try:
            float(FGF13)
            float(GDF3)
            float(SKIL)
        ...
        # checking for the value between 0 and 100
        if 100 >= float(FGF13) >= 0 and 100 >= float(GDF3) >= 0 and 100 >= float(SKIL) >= 0 and 100 >= float(ERAS) >= \
                0 and 100 >= float(TRIM28) >= 0 and 100 >= float(ZFX) >= 0:
            issue = False
        else:
            issue = True
        ...
        # converting from percentage to value from 0-1 which our model can process
        ...
        # inserting these datapoints into a dataframe
        ...
        # using the keras model to predict the probability, a value from 0 to 1
        predictions = model.predict(dataframe).flatten()

        # converting this to a percentage
        percentage = 100 * predictions[0]

        # displaying the correct message for each context
...
# returning the correct HTML for each of the following other pages
@app.route("/")
def index():
